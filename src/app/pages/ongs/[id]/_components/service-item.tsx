...
const ServiceItem = ({ donation, ong, isAuthenticated }: ServiceItemProps) => {
  ...
  const [donationAmount, setDonationAmount] = useState("0,00"); // Estado para o valor da doação formatado
  const [modalIsOpen, setModalIsOpen] = useState(false); // Estado para controlar a abertura do modal de doação
  const [ticketUrl, setTicketUrl] = useState(""); // Estado para a URL do ticket Pix
  ...
  const handleDonationSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/criar-pix", {
        ...
        body: JSON.stringify({
          transaction_amount: valueInCents / 100, // Envia o valor da doação em reais
      ...
      if (response.ok) {
        const { point_of_interaction } = await response.json(); // Extrai os dados do ponto de interação
        setTicketUrl(point_of_interaction.transaction_data.ticket_url); // Define a URL do ticket Pix
      } else {
        const errorText = await response.text();
        console.error("Erro ao criar preferência de pagamento", errorText);
        alert("Erro ao criar preferência de pagamento: " + errorText); // Exibe mensagem de erro ao criar a preferência de pagamento
  ...
  return (
    <Card>
      ...
      {ticketUrl && (
        <Card className="mt-4">
          <CardContent>
            <iframe src={ticketUrl} width="100%" height="500px" />
          </CardContent>
        </Card>
      )}
    </Card>
